...
func main() {
	...
	_ = repo.ApplyRetention(ctx, cfg.RetentionDays)
	...
	if fixturePath := os.Getenv("FLOWSCOPE_SEED_FIXTURE_PATH"); fixturePath != "" {
		fileData, err := os.ReadFile(fixturePath)
		if err == nil {
			var fixture []model.FlowRecord
			if err := json.Unmarshal(fileData, &fixture); err == nil {
				for _, rec := range fixture {
					...
					if len(batch) >= 2000 {
						if err := repo.InsertFlows(ctx, batch); err != nil {
							log.Fatalf("fixture insert failed: %v", err)
						}
						batch = batch[:0]
					}
				}
